...
class PokemonEmeraldLocation(Location):
    ...
    def __init__(
        ...
        self.default_item_code = (
            None
            if default_item_value is None
            else offset_item_value(default_item_value)
        )
    ...
    for region_data in data.regions.values():
        for location_name in region_data.locations:
            ...
            if location_data.flag == 0:
                label_to_id_map[location_data.label] = (
                    BASE_OFFSET + POKEDEX_OFFSET + int(location_data.name[15:])
                )
            else:
...
def set_free_fly(world: "PokemonEmeraldWorld") -> None:
    ...
    if world.options.free_fly_location:
        blacklisted_locations = set(
            BLACKLIST_OPTION_TO_VISITED_EVENT[city]
            for city in world.options.free_fly_blacklist.value
        )
    ...
    free_fly_location_location = world.multiworld.get_location(
        "FREE_FLY_LOCATION", world.player
    )
...
def set_legendary_cave_entrances(world: "PokemonEmeraldWorld") -> None:
    ...
    terra_cave_location_location = world.multiworld.get_location(
        "TERRA_CAVE_LOCATION", world.player
    )
    terra_cave_location_location.item = None
    terra_cave_location_location.place_locked_item(
        world.create_event(terra_cave_location_name)
    )
    ...
    marine_cave_location_location = world.multiworld.get_location(
        "MARINE_CAVE_LOCATION", world.player
    )
    marine_cave_location_location.item = None
    marine_cave_location_location.place_locked_item(
        world.create_event(marine_cave_location_name)
    )
